    ...
    def handle_client(self, client):
        ...
        if locatedResource:  # Respond With Resource
            ...
            if ftype == "py":
                ...
                if (_dscript_end - _dscript_start) * 1000 > 30:
                    print(
                        f"Http Server Warning: Dynamic Script {url} took over 30ms. Time Took {1000*(_dscript_end - _dscript_start)}"
                    )
        ...
        if (_request_end - _request_start) * 1000 > 100:
            print(
                f"Http Server: Performance Warning Request Took Over 100 ms. Time Taken: {1000*(_request_end-_request_start)}"
            )
        ClientSocket.close()
        if self.debug:
